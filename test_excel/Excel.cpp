...
void Excel::close(bool ifSave)
{
	...
	if (!openFileName.IsEmpty())
	{
		...
		{
			//show(true);  
			workBook.Close(COleVariant(short(TRUE)), COleVariant(openFileName), covOptional);
			books.Close();
		}
		else
		{
...
void Excel::setCellDouble(long iRow, long iColumn, double newDouble)
{
	COleVariant new_value(newDouble);
	CRange start_range = workSheet.get_Range(COleVariant(_T("A1")), covOptional);
	CRange write_range = start_range.get_Offset(COleVariant((long)iRow - 1), COleVariant((long)iColumn - 1));
	write_range.put_Value2(new_value);
	start_range.ReleaseDispatch();
